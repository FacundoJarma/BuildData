...
} from "@/services/mock/presupuestoService";
...
export function ScreenPresupuesto() {
  ...
  const totalCap = useMemo(
    () => displayLines.reduce((s, l) => s + l.cap, 0),
    [displayLines],
  );
  const totalSpent = useMemo(
    () => displayLines.reduce((s, l) => s + l.spent, 0),
    [displayLines],
  );
  const totalComp = useMemo(
    () => displayLines.reduce((s, l) => s + l.comp, 0),
    [displayLines],
  );
  const disponible = totalCap - totalSpent;
  const ejecPct = totalCap > 0 ? Math.round((totalSpent / totalCap) * 100) : 0;

  const { projected, totalProjected, riskRubros } =
    useProjections(displayLines);
  const predPct =
    totalCap > 0
      ? Math.min(100, Math.round((totalProjected / totalCap) * 100))
      : 0;
        ...
        <DCard padding="p-0">
          ...
          <div className="divide-y divide-slate-100">
            {projected.map((l, i) => {
              ...
              const usedPct =
                totalCap > 0 ? Math.round((l.spent / l.cap) * 100) : 0;
              const compPct =
                totalCap > 0 ? Math.round((l.comp / l.cap) * 100) : 0;
                  ...
                  <div className="flex items-center gap-3 mb-1.5">
                    {editMode ? (
                      <input
                        ...
                        value={l.cap}
                        onChange={(e) =>
                          handleCapChange(i, Number(e.target.value))
                        }
                      ...
                      <span className="text-[12px] font-bold tnum text-slate-950">
                        {fmt(l.spent)}
                      </span>
                    )}
        ...
        <div className="space-y-4">
          <DCard padding="p-4">
            ...
            <div className="space-y-0">
              {FORECAST_ITEMS.map((f, i) => (
                <div
                  key={i}
                  className="flex gap-3 pb-3 border-l-2 border-slate-200 pl-3 ml-1 relative"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] font-bold tnum text-primary mt-1">
                      {fmt(f.amount)}
                    </div>
                  </div>
                </div>
          ...
          <DCard padding="p-4">
            ...
            <div className="space-y-2.5">
              {projected.map((l, i) => {
                const pct =
                  totalCap > 0 ? Math.round((l.cap / totalCap) * 100) : 0;
                const color = RUBRO_COLORS[i % RUBRO_COLORS.length];
                return (
                  <div key={l.name}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] font-bold tnum text-slate-500 ml-2">
                        {pct}%
                      </span>
                    </div>
