...
export function NewSuccessState({ data, obra, onClose }: { data: any; obra?: any; onClose: () => void }) {
  const tipoLabel = TIPO_LABELS[data.tipo as string] || "obra";
  const displayName = obra?.name || data.nombre || "Tu nueva obra";
  const teamSize = obra?.team?.length ?? (data.team || []).filter((m: any) => m.name.trim()).length + 1;
  ...
  return (
    <div className="text-center py-6">
      ...
      <h3 className="text-[24px] font-extrabold display-tight">{displayName} está lista</h3>
      ...
      <div className="flex gap-2 justify-center">
        ...
        <a
          href={obra ? `/dashboard?id=${obra.id}` : "/dashboard"}
          className="inline-flex items-center gap-2 bg-primary hover:bg-primary-700 text-white font-bold rounded-md px-5 py-[10px] text-[13px] transition-colors"
        >
